...
def parse_args(argv=None):
    ...
    parser.add_argument('--check', dest='check', action='store_true',
                        help='do not format files inplace, but check',
                        default=False)
...
def run_format(args, task_queue, formatted_files):
    ...
    while True:
        ...
        formatted = subprocess.check_output(invocation, stderr=subprocess.STDOUT, text=True)
...
def main():
    ...
    result = format_all(args, files)
    ret_value = 0
    for file, out in result.items():
        if out:
            print(out)
            ret_value = 1
    sys.exit(ret_value)
