...
static string good_diversion_hdrs =
    "Diversion: " + good_na_1 + CRLF + "Diversion: " + good_na_2 + ", " + good_na_3 + CRLF + "Diversion: " + good_na_4;
...
{
    *os << "[" << arg.varformat << "," << arg.varparam << "," << arg.expected_result.print();
}

class UsedHeaderFieldTest : public testing::TestWithParam<tparam> {};
...
static AmArg uri_json_expected({
    ...
    { "up_arr", { "uparam1=uval11", "uparam2=uval12", "uparam3" } },
    {     "uh",
     { {
     { "uhdr1", "uhval1" },
     } }                                                         },
...
static AmArg uri_json_array_expected{
    ...
    AmArg{
          ...
          { "h", "domain2" },
          { "p", 5061 },
          { "up", { { "uparam1", "uval21" }, { "uparam2", "uval22" } } },
          { "up_arr", { "uparam1=uval21", "uparam2=uval22" } },
          },
    AmArg{
          { "n", "test3" },
          { "s", "sips" },
          ...
          { "h", "domain3" },
          { "p", 5062 },
          { "up", { { "uparam1", "uval31" }, { "uparam2", "uval32" } } },
          { "up_arr", { "uparam1=uval31", "uparam2=uval32" } },
          },
    AmArg{ { "s", "tel" }, { "u", "1234567890" } }
};

INSTANTIATE_TEST_SUITE_P(
    YetiTest, UsedHeaderFieldTest,
    testing::Values(tparam("", "", good_na_1 + ", " + good_na_2 + ", " + good_na_3 + ", " + good_na_4),
                    tparam("uri_user", "", "user1"), tparam("uri_user_array", "", "user1,user2,user3,1234567890"),
                    tparam("uri_domain", "", "domain1"), tparam("uri_domain_array", "", "domain1,domain2,domain3,"),
                    tparam("uri_port", "", "5060"), tparam("uri_port_array", "", "5060,5061,5062,0"),
                    tparam("uri_param", "uparam1", "uval11"),
                    tparam("uri_param_array", "uparam1", "uval11,uval21,uval31,"),
                    tparam("uri_json", "", uri_json_expected), tparam("uri_json_array", "", uri_json_array_expected)),

    [](const testing::TestParamInfo<UsedHeaderFieldTest::ParamType> &info) {
        string ret;

        if (info.param.varformat.empty())
            ret = "raw";
        else
            ret = info.param.varformat;
