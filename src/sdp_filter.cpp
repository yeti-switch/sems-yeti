...
static const SdpPayload *findPayload(
    const std::vector<SdpPayload>& payloads,
    const SdpPayload &payload,
    int transport)
{
...
int filterNoAudioStreams(AmSdp &sdp, bool filter){
    ...
    for (std::vector<SdpMedia>::iterator m_it = sdp.media.begin();
        m_it != sdp.media.end(); m_it++)
    {
        SdpMedia& media = *m_it;
        if(media.type!=MT_AUDIO) {
            media.port = 0;
...
inline void add_codec(
    std::vector<SdpPayload> &pl,
    const SdpPayload &p,bool single_codec)
{
 ...
 */
void replaceDynamicPayloads(
    std::vector<SdpPayload> &dst_payloads,
    const std::vector<SdpPayload> *ref_payloads = nullptr,
    int transport = 0)
{
    ...
    for(auto &dst_pl : dst_payloads) {
        if(dst_pl.payload_type < 0) {
            ...
            DBG("assign dynamic payload id for %s/%d: %d -> %d",
                dst_pl.encoding_name.c_str(), dst_pl.clock_rate,
        ...
        if(conflicting_dst_pl_it != dst_used_dynamic_payloads.end()) {
            ...
            DBG("replace dynamic payload id for %s/%d: %d -> %d "
                ...
                conflicting_pl.encoding_name.c_str(), conflicting_pl.clock_rate,
...
void fixDynamicPayloads(
    AmSdp &sdp,
    const vector<SdpMedia> *reference_media)
{
    auto audio_stream_predicate = [](const SdpMedia& m) -> bool {
        return m.type == MT_AUDIO;
    };
    ...
    if(reference_media && reference_media->size()) {
        ...
        ref_it = std::find_if(
            media.begin(), media.end(), audio_stream_predicate);
    }
    ...
    for(auto &media : sdp.media) {
        ...
        if(reference_media && ref_it != reference_media->end()) {
            replaceDynamicPayloads(
                media.payloads, &ref_it->payloads, media.transport);

            ++ref_it;
            //skip no-audio streams in the negotiated_media
            ref_it = std::find_if(
                ref_it, reference_media->end(), audio_stream_predicate);
        } else {
