...
template <> void send_dtmf<DTMF>(AmSipDialog *dlg, AmDtmfEvent *dtmf)
{
    AmMimeBody    body;
    AmContentType type;
...
template <> void send_dtmf<DTMF_RELAY>(AmSipDialog *dlg, AmDtmfEvent *dtmf)
{
    AmMimeBody    body;
    AmContentType type;
...
{
    switch (m_type) {
    case DTMF:       send_dtmf<DTMF>(dlg, this); break;
    case DTMF_RELAY: send_dtmf<DTMF_RELAY>(dlg, this); break;
    default:         ERROR("unknown dtmf sip info event type");
    }
}
