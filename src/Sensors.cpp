		...
		if(isArgArray(data)) {
			for(size_t i = 0; i < data.size(); i++) {
				...
				case sensor::SENS_TYPE_IPIP: {
					...
					if(ipip_sensor->init(
						DbAmArg_hash_get_str(row,"o_source_ip").data(),
						DbAmArg_hash_get_str(row,"o_target_ip").data(),
						NULL))
					{
				...
				case sensor::SENS_TYPE_ETHERNET: {
					...
					if(ethernet_sensor->init(
						DbAmArg_hash_get_str(row,"o_source_interface").data(),
