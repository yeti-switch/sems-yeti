...
bool add2hash(cfg_t *c, std::string key, std::string cfg_key, AmConfigReader &out)
{
    ...
    case CFGT_BOOL:
        ...
        break;
    default: ERROR("uknown option type: %d for key: '%s'", opt->type, cfg_key.c_str());
    }
    return true;
}

void apply_db_cfg(cfg_t *c, std::string prefix, AmConfigReader &out)
{
    ...
    add2hash(c, prefix + "keepalives_interval", "keepalives_interval", out);
}

void apply_pool_cfg(cfg_t *c, std::string prefix, AmConfigReader &out)
{
    // apply_db_opts
