...
{
    if (argc != 2 && argc != 4) {
        ERROR("unexpected option args count for header(). "
              "expected formats: "
    ...
    if (argc == 4) {
        if (cfg_aleg_cdr_headers.add_snapshot_header(argv[0], argv[2], argv[3])) {
            return 1;
        }
    }
...
{
    add_sip_reason = cfg_getbool(cfg, opt_name_cdr_headers_add_sip_reason);

    add_q850_reason = cfg_getbool(cfg, opt_name_cdr_headers_add_q850_reason);
}
...
int YetiCfg::configure(cfg_t *cfg, AmConfigReader &am_cfg)
{
    ...
    write_auth_error_id            = cfg_getbool(cfg, opt_name_write_auth_error_id);
