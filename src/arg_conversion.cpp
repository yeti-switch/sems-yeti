...
static bool string2arg(const char *&src, int &len, AmArg &dst)
{
    ...
    case ARRAY_LABEL:
        dst.assertArray();
        if (!read_len(++src, --len, cnt))
            return false;
        for (int i = 0; i < cnt; i++) {
            dst.push(AmArg());
            if (!string2arg(src, len, dst.get(dst.size() - 1)))
                return false;
        }
        return true;
    ...
    case STRUCT_LABEL:
        ...
        for (int i = 0; i < cnt; i++) {
            if (!read_string(src, len, s))
                return false;
            dst[s] = AmArg();
            if (!string2arg(src, len, dst[s]))
                return false;
        }
        return true;

    default: DBG("unknown label '%c'", src[0]); return false;
    }
}
