...
void ResourceList::parse(const std::string &rs)
{
    ...
    for (vector<string>::const_iterator ri = lc.begin(); ri != lc.end(); ++ri) {
        // DBG("   *ri = %s",(*ri).c_str());
        vector<string> ac = explode(*ri, RES_OPTION_SEPARATOR);
        for (vector<string>::const_iterator ai = ac.begin(); ai != ac.end(); ++ai) {
            // DBG("      *ai = %s",(*ai).c_str());
            Resource       r;
            ...
            r.id = vc[1];
            if (str2int(vc[0], r.type) && str2int(vc[2], r.limit) && str2int(vc[3], r.takes)) {
        ...
        if (!this->empty())
            this->back().failover_to_next = false;
    }
}
