...
CCChainProcessing YetiCC::onInitialInvite(SBCCallLeg *call, InitialInviteHandlerParams &params) {
	...
	if(config.early_100_trying){
		msg_logger *logger = call->getLogger();
		if(logger){
			ctx->early_trying_logger->relog(logger);
		}
	} else {
		call->dlg->reply(req,100,"Connecting");
	}

	if(!radius_auth(call,*cdr,call_profile,req)){
		onRoutingReady(call,req,*params.modified_invite);
	}
