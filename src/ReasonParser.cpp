...
void ReasonParser::parse_reason(const std::string &hdrs, size_t reason_begin, size_t reason_end)
{
    ...
    char              last_c = 0;
    string::size_type name_start_pos{}, name_end_pos{}, value_start_pos{}, value_end_pos{};
    enum {
        ST_START,
    ...
    auto on_name_value = [&]() -> bool {
        ...
        if (0 == hdrs.compare(name_start_pos, name_len, reason_param_cause)) {
            if (str2int(hdrs.substr(value_start_pos, value_end_pos - value_start_pos), current_reason->cause)) {
    ...
    while (pos < reason_end) {
        ...
        case '\"':
            ...
            case ST_ESCAPED_VALUE:
                ...
                break;
            default: ERROR("unexpected dquote at %lu. hdr: %s", pos, hdrs.data());
            }
            break;
        case '=':
            switch (st) {
            ...
            default:               ERROR("unexpected equal sign at %lu. hdr: %s", pos, hdrs.data()); return;
            }
            break;
        case ';':
        ...
        case ' ':
            ...
            case ST_VALUE_SKIP_WS_AFTER:
            case ST_ESCAPED_VALUE:        break;
            default:                      ERROR("unexpected space at %lu. hdr: %s", pos, hdrs.data()); return;
            }
            break;
        default:
            switch (st) {
            ...
            case ST_VALUE:
            case ST_NAME:          break;
            default:               ERROR("unexpected '%c' at %lu. hdr: %s", c, pos, hdrs.data()); return;
            }
            break;
        } // switch(c)
        pos++;
...
{
    return (sip_reason.parsed && cfg.add_sip_reason) || (q850_reason.parsed && cfg.add_q850_reason);
}

void ReasonParser::serialize(AmArg &ret, const YetiCfg::headers_processing_config::leg_reasons &cfg)
{
...
inline bool validate_cause(int cause)
{
    ...
    return cause >= std::numeric_limits<int16_t>::min() && cause <= std::numeric_limits<int16_t>::max();
}

void ReasonParser::serialize_flat(AmArg &ret, const YetiCfg::headers_processing_config::leg_reasons &cfg,
                                  const string &local_tag)
{
    if (sip_reason.parsed && cfg.add_sip_reason) {
        if (validate_cause(sip_reason.cause)) {
            ...
            WARN("[%s] SIP cause value %d is out of range for pg type smallint. use null", local_tag.data(),
                 sip_reason.cause);
            ret["sip_cause"] = AmArg();
        }
    ...
    if (q850_reason.parsed && cfg.add_q850_reason) {
        if (validate_cause(q850_reason.cause)) {
            ...
            WARN("[%s] Q850 cause value %d is out of range for pg type smallint. use null", local_tag.data(),
                 q850_reason.cause);
