...
int filterSDP(AmSdp &sdp, const vector<FilterEntry> &filter_list)
{
    ...
    for (vector<FilterEntry>::const_iterator it = filter_list.begin(); it != filter_list.end(); it++) {
        ...
        for (std::vector<SdpMedia>::iterator m_it = sdp.media.begin(); m_it != sdp.media.end(); m_it++) {
            ...
            for (std::vector<SdpPayload>::iterator p_it = media.payloads.begin(); p_it != media.payloads.end(); p_it++)
            {

                string c = p_it->encoding_name;
                std::transform(c.begin(), c.end(), c.begin(), ::tolower);

                bool is_filtered = (sdpfilter == Whitelist) ^ (sdpfilter_list.find(c) != sdpfilter_list.end());
...
{
    for (std::vector<SdpPayload>::iterator p_it = m.payloads.begin(); p_it != m.payloads.end(); p_it++) {
        SdpPayload &p = *p_it;
        ...
        DBG("named SDP payload type %d with %s/%d%s", p.payload_type, IANA_RTP_PAYLOADS[p.payload_type].payload_name,
...
{
    for (std::vector<SdpAttribute>::iterator a_it = m.attributes.begin(); a_it != m.attributes.end(); a_it++) {
        if (a_it->attribute == "silenceSupp") {
            ...
            if (parts.size() < 5) {
                ...
                for (int i = parts.size(); i < 5; i++)
                    a_it->value += " -";
                DBG("fixed SDP attribute silenceSupp:'%s' -> '%s'", val_before.c_str(), a_it->value.c_str());
            }
        }
    }
}

std::vector<SdpAttribute> filterSDPAttributes(std::vector<SdpAttribute> attributes, FilterType sdpalinesfilter,
                                              const std::set<string> &sdpalinesfilter_list)
{

    std::vector<SdpAttribute> res;
    for (std::vector<SdpAttribute>::iterator a_it = attributes.begin(); a_it != attributes.end(); a_it++) {
        ...
        bool is_filtered =
            (sdpalinesfilter == Whitelist) ^ (sdpalinesfilter_list.find(c) != sdpalinesfilter_list.end());

        DBG("%s (%s) is_filtered: %s", a_it->attribute.c_str(), c.c_str(), is_filtered ? "true" : "false");
...
int filterSDPalines(AmSdp &sdp, const vector<FilterEntry> &filter_list)
{
    for (vector<FilterEntry>::const_iterator it = filter_list.begin(); it != filter_list.end(); it++) {
        ...
        sdp.attributes = filterSDPAttributes(sdp.attributes, sdpalinesfilter, sdpalinesfilter_list);
