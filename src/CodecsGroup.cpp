...
static bool readPayload(SdpPayload &p, const string &src)
{
    ...
    if (elems.size() > 2)
        str2int(elems[2], p.encoding_param);
    if (elems.size() > 1)
...
bool CodecsGroupEntry::add_codec(string c, string sdp_params, int dyn_payload_id)
{
    ...
    if (!payload) {
        DBG("Ignoring unknown payload: %s/%i", p.encoding_name.c_str(), p.clock_rate);
    ...
    if (payload_id < DYNAMIC_PAYLOAD_TYPE_START) {
        ...
        if (dyn_payload_id != -1) {
            DBG("found dyn_payload_id %d for codec %s/%i", dyn_payload_id, p.encoding_name.c_str(), p.clock_rate);
            if (dyn_payload_id < DYNAMIC_PAYLOAD_TYPE_START) {
                ERROR("Ignoring dyn_payload_id %d for %s/%i. it mustn't be less than %d", dyn_payload_id,
                      p.encoding_name.c_str(), p.clock_rate, DYNAMIC_PAYLOAD_TYPE_START);
                p.payload_type = -1;
            } else {
