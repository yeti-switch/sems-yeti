...
#define PROF_START(var)                                                                                                \
    timeval prof_start_##var;                                                                                          \
    gettimeofday(&prof_start_##var, nullptr);
#define PROF_END(var)                                                                                                  \
    timeval prof_end_##var;                                                                                            \
    gettimeofday(&prof_end_##var, nullptr);
#define PROF_DIFF(var)                                                                                                 \
    timeval prof_diff_##var;                                                                                           \
    timersub(&prof_end_##var, &prof_start_##var, &prof_diff_##var);
#define PROF_PRINT(descr, var)                                                                                         \
    PROF_DIFF(var);                                                                                                    \
    DBG("PROFILING: " descr " took %s", timeval2str_usec(prof_diff_##var).c_str());
...
class YetiComponentInited : public AmEvent {
  public:
    enum ComponentType { Resource = 0, MaxType } type;
    YetiComponentInited(ComponentType type)
        : AmEvent(0)
        , type(type)
    {
    }
...
struct YetiBase {
    ...
    struct sync_db {
        enum DbReplyResult { DB_REPLY_WAITING = 0, DB_REPLY_RESULT, DB_REPLY_ERROR, DB_REPLY_TIMEOUT };
