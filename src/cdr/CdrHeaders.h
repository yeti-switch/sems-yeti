...
class cdr_headers_t {
    ...
    struct SnapshotHeaderData {
        ...
        string                                   snapshot_key;
        SnapshotHeaderData(cdr_header_snapshot_serialization_type_t type, string snapshot_key)
            : type(type)
  ...
  public:
    ...
    int   add_snapshot_header(std::string header_name, const std::string &snapshot_key,
