...
#define timeriseq(a, b) (((a).tv_sec == (b).tv_sec) && ((a).tv_usec == (b).tv_usec))
...
{
    static const char *DisconnectInitiatorStr[] = { "Database", "TrafficSwitch", "Destination", "Originator",
                                                    "Undefined" };
...
void Cdr::update_with_bleg_sip_request(const AmSipRequest &req)
{
    ...
    bleg_headers_amarg = Yeti::instance().config.bleg_cdr_headers.serialize_headers(req.hdrs);
}
...
void Cdr::update_init_bleg(const string &leg_term_call_id, const string &leg_local_tag)
{
    if (writed)
        return;
...
void Cdr::update_with_action(UpdateAction act)
{
    ...
    case Connect:
        if (timeriseq(start_time, end_time)) {
            ...
            WARN("%s: attempt to set connect_time after the end_time", local_tag.data());
        }
        break;
    case BlegConnect: gettimeofday(&bleg_connect_time, NULL); break;
    case End:
        if (timeriseq(start_time, end_time))
...
{
    DBG3("Cdr::%s(reason = '%s',code = %d)", FUNC_NAME, reason.c_str(), code);
...
void Cdr::update_internal_reason(DisconnectInitiator initiator, const string &reason, unsigned int code,
                                 unsigned int internal_code_id)
{
    DBG3("Cdr[%p]::%s(initiator = %d,reason = '%s',code = %d) cdr.disconnect_initiator_writed = %d", this, FUNC_NAME,
         initiator, reason.c_str(), code, disconnect_initiator_writed);
...
{
    if (a_leg)
        aleg_sdp_completed = true;
    else
        bleg_sdp_completed = true;
}

static string join_str_vector2(const vector<string> &v1, const vector<string> &v2, const string &delim)
{
    std::stringstream ss;
...
#define serialize_math_stat(j, PREFIX, STAT)                                                                           \
    ...
    for (auto &rx : m.rx) {
        ...
        cJSON_AddStringToObject(rx_json, "rx_payloads_transcoded", join_vector(rx.payloads_transcoded, ',').c_str());
        cJSON_AddStringToObject(rx_json, "rx_payloads_relayed", join_vector(rx.payloads_relayed, ',').c_str());
    ...
    cJSON_AddStringToObject(j, "tx_payloads_transcoded", join_vector(m.tx.payloads_transcoded, ',').c_str());
    cJSON_AddStringToObject(j, "tx_payloads_relayed", join_vector(m.tx.payloads_relayed, ',').c_str());
...
#define add_tv2json(value)                                                                                             \
    if (timerisset(&value))                                                                                            \
        cJSON_AddNumberToObject(j, field_name, timeval2double(value));                                                 \
    else                                                                                                               \
        cJSON_AddNullToObject(j, field_name)

    int                i = 0;
    cJSON             *j;
    char              *s;
    static const char *fields[] = {
        "lega_rx_payloads",    "lega_tx_payloads",

        "legb_rx_payloads",    "legb_tx_payloads",

        "lega_rx_bytes",       "lega_tx_bytes",

        "legb_rx_bytes",       "legb_tx_bytes",

        "lega_rx_decode_errs", "lega_rx_no_buf_errs", "lega_rx_parse_errs",

        "legb_rx_decode_errs", "legb_rx_no_buf_errs", "legb_rx_parse_errs",
    };

    j = cJSON_CreateObject();

    // tx/rx uploads
    add_str2json(join_str_vector2(aleg_rx_payloads_transcoded, aleg_rx_payloads_relayed, ",").c_str());

    add_str2json(join_str_vector2(aleg_tx_payloads_transcoded, aleg_tx_payloads_relayed, ",").c_str());

    add_str2json(join_str_vector2(bleg_rx_payloads_transcoded, bleg_rx_payloads_relayed, ",").c_str());

    add_str2json(join_str_vector2(bleg_tx_payloads_transcoded, bleg_tx_payloads_relayed, ",").c_str());
...
char *Cdr::serialize_timers_data()
{
    ...
    static const char *fields[] = { "time_start", "leg_b_time", "time_connect", "time_end",
                                    "time_1xx",   "time_18x",   "time_limit",   "isup_propagation_delay" };
...
char *Cdr::serialize_dtmf_events()
{
    ...
    while (!dtmf_events_a2b.empty()) {
        cJSON_AddItemToArray(a, dtmf_events_a2b.front().serialize2json(t));
        dtmf_events_a2b.pop();
    }
    ...
    while (!dtmf_events_b2a.empty()) {
        cJSON_AddItemToArray(a, dtmf_events_b2a.front().serialize2json(t));
        dtmf_events_b2a.pop();
    }
...
char *Cdr::serialize_dynamic(const DynFieldsT &df)
{
    ...
    for (auto const &f : df) {
        ...
        switch (arg.getType()) {
        case AmArg::Int: cJSON_AddItemToObject(j, namep, cJSON_CreateNumber(static_cast<double>(arg.asInt()))); break;
        case AmArg::LongLong:
            cJSON_AddItemToObject(j, namep, cJSON_CreateNumber(static_cast<double>(arg.asLongLong())));
            break;
        case AmArg::Bool:   cJSON_AddBoolToObject(j, namep, arg.asBool()); break;
        case AmArg::CStr:   cJSON_AddStringToObject(j, namep, arg.asCStr()); break;
        case AmArg::Double: cJSON_AddNumberToObject(j, namep, arg.asDouble()); break;
        case AmArg::Array:  cJSON_AddStringToObject(j, namep, arg2json(arg).data());
        case AmArg::Undef:  cJSON_AddNullToObject(j, namep); break;
        default:
            ERROR("invoc_AmArg. unhandled AmArg type %s", arg.t2str(arg.getType()));
...
void Cdr::apply_params(QueryInfo &query_info, const DynFieldsT &)
{
...
#define invoc_json(func)                                                                                               \
    ...
    if (cfg.write_internal_disconnect_code) {
        invoc_cond_typed("smallint", disconnect_internal_code_id, disconnect_internal_code_id != 0);
    }
    ...
    if (failed_resource_id.empty()) {
        ...
        if (str2longlong(failed_resource_id, failed_resource_id_bigint)) {
            ...
            DBG("[%s] failed to write failed_resource_id:'%s' as bigint. use null", local_tag.data(),
                failed_resource_id.data());
            invoc_null();
        }
    }
    ...
    if (aleg_reasons.has_data(cfg.headers_processing.aleg)) {
        ...
        aleg_reasons.serialize_flat(aleg_headers_amarg["reason"], cfg.headers_processing.aleg, local_tag);
    }
    ...
    if (bleg_reasons.has_data(cfg.headers_processing.bleg)) {
        bleg_reply_headers_amarg.assertStruct();
        bleg_reasons.serialize_flat(bleg_reply_headers_amarg["reason"], cfg.headers_processing.bleg, local_tag);
    }
    invoc_cond(arg2json(bleg_reply_headers_amarg),
               isArgStruct(bleg_reply_headers_amarg) && bleg_reply_headers_amarg.size());
...
void Cdr::snapshot_info_filtered(AmArg &s, const DynFieldsT &df, const unordered_set<string> &wanted_fields) const
{
    static char      strftime_buf[64] = { 0 };
    static struct tm tt;
...
#define add_field(val)          a[#val] = val;
#define add_field_as(name, val) a[name] = val;
#define add_timeval_field(val)  a[#val] = timerisset(&val) ? timeval2double(val) : AmArg();
