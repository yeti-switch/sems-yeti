...
template <enum dtmf_info_mode method> void send_dtmf(AmSipDialog *dlg, AmDtmfEvent *dtmf);
...
class DtmfInfoSendEvent : public AmDtmfEvent {
  ...
  public:
    DtmfInfoSendEvent(int event, int duration, dtmf_info_mode type)
        : AmDtmfEvent(event, duration, -1, Dtmf::SOURCE_DETECTOR)
        , m_type(type)
    {
    ...
    DtmfInfoSendEvent(AmDtmfEvent *e, dtmf_info_mode type)
        : AmDtmfEvent(e->event(), e->duration(), e->volume(), Dtmf::SOURCE_DETECTOR)
        , m_type(type)
    {
        event_id = B2BDtmfEvent;
    }

    dtmf_info_mode type() { return m_type; }
