...
int DbConfig::cfg2dbcfg(AmConfigReader& cfg, const string& prefix, bool silent)
{
  ...
  string var_prefix = format("{}.{}{}",
    is_cdr_cfg ? "cdr" : "routing",
  ...
  if(cfg.hasParameter(var)){
    ...
    host="127.0.0.1";
    if(!silent) WARN("missed %s.host. use: %s", var_prefix.c_str(), host.c_str());
  }
  ...
  if(cfg.hasParameter(var)){
    ...
    port=5432;
    if(!silent) WARN("missed %s.port. use: %d", var_prefix.c_str(), port);
  }
  var=prefix+"_name";
  if(cfg.hasParameter(var)){
    ...
    if(!silent) WARN("missed %s.name. use: %s", var_prefix.c_str(), name.c_str());
  }
  var=prefix+"_user";
  if(cfg.hasParameter(var)){
    ...
    if(!silent) WARN("missed %s.user. use: %s", var_prefix.c_str(), user.c_str());
  }
  var=prefix+"_pass";
  if(cfg.hasParameter(var)){
