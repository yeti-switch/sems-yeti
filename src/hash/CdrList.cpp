...
long int CdrList::getCallsCount()
{
    ...
    AmEventDispatcher::instance()->iterate([&](const string &, const AmEventDispatcher::QueueEntry &entry) {
...
int CdrList::init_supported_fields()
{
    ...
    for (const auto &df : router_dyn_fields) {
        if (supported_fields.hasMember(df.name)) {
            ERROR("routing dynamic field '%s' conflict. existent field: %s", df.name.data(),
    ...
    for (const auto &[hdr_name, data] : cfg.aleg_cdr_headers.get_snapshot_headers()) {
        if (supported_fields.hasMember(data.snapshot_key)) {
            ERROR("aleg_cdr_headers field key '%s' conflict. existent field: %s", data.snapshot_key.data(),
...
int CdrList::configure(cfg_t *confuse_cfg)
{
    ...
    if (!cfg_size(clickhouse_sec, opt_name_destinations) || !cfg_size(clickhouse_sec, opt_name_table)) {
    ...
    for (unsigned int i = 0; i < cfg_size(clickhouse_sec, opt_name_destinations); i++) {
        snapshots_destinations.emplace_back(cfg_getnstr(clickhouse_sec, opt_name_destinations, i));
    }

    for (unsigned int i = 0; i < cfg_size(clickhouse_sec, opt_name_allowed_fields); i++) {
        snapshots_fields_whitelist.emplace(cfg_getnstr(clickhouse_sec, opt_name_allowed_fields, i));
    }

    DBG("use table '%s' for active calls snapshots "
        "with interval %d (seconds). "
        "buffering is %sabled",
        snapshots_table.c_str(), snapshots_interval, snapshots_buffering ? "en" : "dis");
    ...
    if (snapshots_buffering) {
        ...
        while (!local_postponed_calls.empty()) {
            ...
            if (snapshots_fields_whitelist.empty()) {
                ...
                call[end_time_key] = timerisset(&cdr.end_time) ? timeval2str(cdr.end_time) : AmArg();
            } else {
...
void CdrList::sendSnapshot(const AmArg &calls)
{
    ...
    for (const auto &destination : snapshots_destinations) {
        if (!AmSessionContainer::instance()->postEvent(HTTP_EVENT_QUEUE,
                                                       new HttpPostEvent(destination, data, string())))
        {
...
{
#define add_field(val)         arg[#val] = cdr->val;
#define add_timeval_field(val) arg[#val] = timeval2double(cdr->val);
...
{
#define filter(val)            if (::find(wanted_fields.begin(), wanted_fields.end(), val) != wanted_fields.end())
#define add_field(val)         filter(#val) arg[#val] = cdr->val;
#define add_timeval_field(val) filter(#val) arg[#val] = timeval2double(cdr->val);
