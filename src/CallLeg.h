...
class CallLeg : public AmB2BSession {
  public:
    ...
    enum CallStatus {
        ...
        Connected, //< there is exactly one call leg we are connected to, in this case AmB2BSession::other_id holds the
                   // other leg id
  ...
  private:
    ...
    virtual void onCallStatusChange([[maybe_unused]] const StatusChangeCause &cause) {}
    ...
    virtual void onCallFailed([[maybe_unused]] CallFailureReason reason, [[maybe_unused]] const AmSipReply *reply) {}
  ...
  public:
    ...
    CallLeg(const CallLeg *caller, AmSipDialog *p_dlg = nullptr, AmSipSubscription *p_subs = nullptr);

    virtual ~CallLeg();

    // OA callbacks
    virtual int  onSdpCompleted(const AmSdp &local, const AmSdp &remote, bool sdp_offer_owner) override;
    virtual bool getSdpOffer([[maybe_unused]] AmSdp &offer) override { return false; }
    virtual bool getSdpAnswer([[maybe_unused]] const AmSdp &offer, [[maybe_unused]] AmSdp &answer) override
    {
        return false;
    }
