...
SBCFactory::SBCFactory(const string &_app_name)
    ...
    , initial_invite_requests_counter(stat_group(Counter, MOD_NAME, "rx_requests")
                                          .setHelp("RX initial INVITEs count")
                                          .addAtomicCounter()
                                          .addLabel("method", "INVITE"))
    , register_requests_counter(
          stat_group(Counter, MOD_NAME, "rx_requests").addAtomicCounter().addLabel("method", "REGISTER"))
    , options_requests_counter(
          stat_group(Counter, MOD_NAME, "rx_requests").addAtomicCounter().addLabel("method", "OPTIONS"))
    , invite_pre_auth_rejects(stat_group(Counter, MOD_NAME, "auth_rejects")
                                  .addAtomicCounter()
                                  .addLabel("method", "INVITE")
                                  .addLabel("mechanism", "ip_auth"))
    , invite_auth_rejects(
          stat_group(Counter, MOD_NAME, "auth_rejects_total").addAtomicCounter().addLabel("method", "INVITE"))
    , invite_digest_auth_rejects(stat_group(Counter, MOD_NAME, "auth_rejects")
                                     .addAtomicCounter()
                                     .addLabel("method", "INVITE")
                                     .addLabel("mechanism", "digest"))
    , invite_jwt_auth_rejects(stat_group(Counter, MOD_NAME, "auth_rejects")
                                  .addAtomicCounter()
                                  .addLabel("method", "INVITE")
                                  .addLabel("mechanism", "jwt"))
    , register_auth_rejects(
          stat_group(Counter, MOD_NAME, "auth_rejects_total").addAtomicCounter().addLabel("method", "REGISTER"))
    , register_digest_auth_rejects(stat_group(Counter, MOD_NAME, "auth_rejects")
                                       .addAtomicCounter()
                                       .addLabel("method", "REGISTER")
                                       .addLabel("mechanism", "digest"))
    , register_jwt_auth_rejects(stat_group(Counter, MOD_NAME, "auth_rejects")
                                    .addAtomicCounter()
                                    .addLabel("method", "REGISTER")
                                    .addLabel("mechanism", "jwt"))
...
void SBCFactory::send_and_log_auth_challenge(const AmSipRequest &req, const OriginationPreAuth::Reply &ip_auth_data,
                                             const string &internal_reason, bool post_auth_log, int auth_feedback_code)
{
    string hdrs;
    if (auth_feedback) {
...
AmSession *SBCFactory::onInvite(const AmSipRequest &req, const string &, const map<string, string> &)
{
    ...
    AmArg ret;
    auto  auth_result_id = yeti->router.check_request_auth(req, ip_auth_data, ret);
    if (auth_result_id > 0) {
        ...
        if (auth_result_id_negated > Auth::NO_IP_AUTH) {
            ...
            send_and_log_auth_challenge(req, ip_auth_data, ret.asCStr(), true, auth_result_id_negated);
        }
        ...
        send_and_log_auth_challenge(req, ip_auth_data, no_auth_internal_reason,
...
void SBCFactory::onOoDRequest(const AmSipRequest &req)
{
    ...
    if (req.method == SIP_METH_REGISTER) {
        ...
        if (auth_id == Auth::NO_AUTH) {
            send_and_log_auth_challenge(req, ip_auth_data, "no Authorization header", true);
            return;
        }
