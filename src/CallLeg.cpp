...
void CallLeg::terminateNotConnectedLegs()
{
    ...
    while (it != other_legs.end()) {
        ...
        AmSessionContainer::instance()->postEvent(it->first, new B2BEvent(B2BTerminateLeg));
...
int CallLeg::relaySipReply(AmSipReply &reply)
{
    ...
    if (t_req == recvd_req.end()) {
        ERROR("%s: Request with CSeq %u not found in recvd_req for %u reply.", getLocalTag().c_str(), reply.cseq,
              reply.code);
        return 0; // ignore?
    }
...
void CallLeg::b2bInitial1xx(AmSipReply &reply, bool forward)
{
    ...
    if (reply.to_tag.empty()) {
        DBG("got %d without to_tag. allow_1xx_without_to_tag = %d", reply.code, allow_1xx_without_to_tag);
...
void CallLeg::b2bInitialErr(AmSipReply &reply, bool forward)
{
    ...
    DBG("clean-up after non-ok reply (reply: %d, status %s, other: %s)", reply.code, callStatus2str(getCallStatus()),
        getOtherId().c_str());
...
void CallLeg::onB2BReply(B2BSipReplyEvent *ev)
{
    ...
    bool initial_reply = (reply.cseq_method == SIP_METH_INVITE && (call_status == NoReply || call_status == Ringing) &&
    ...
    if (initial_reply) {
        ...
        if (call_status == Connected && reply.code >= 200 && reply.cseq_method == SIP_METH_INVITE &&
        ...
        if (getOtherId() != ev->sender_ltag && getOtherId() != reply.from_tag) {
            TRACE("ignoring reply from %s in %s state, other_id = '%s'\n", reply.from_tag.c_str(),
                  callStatus2str(call_status), getOtherId().c_str());
            return;
        }
...
void CallLeg::onB2BConnect(ConnectLegEvent *co_ev)
{
    ...
    int res = dlg->sendRequest(SIP_METH_INVITE, &body, co_ev->hdrs, SIP_FLAGS_VERBATIM, &inv_timers_override,
...
static void sdp2body(const AmSdp &sdp, AmMimeBody &body)
{
    ...
    if (s)
        s->parse(SIP_APPLICATION_SDP, (const unsigned char *)body_str.c_str(), body_str.length());
    else
...
void CallLeg::resumeAccepted()
{
    ...
    AmB2BMedia *ms = getMediaSession();
    if (ms)
        ms->unmute(!a_leg); // unmute the stream in other (!) leg
...
{
    TRACE("%s: SIP request %d %s received in %s state\n", getLocalTag().c_str(), req.cseq, req.method.c_str(),
    ...
    // other leg.
    if ((getCallStatus() == Disconnected || getCallStatus() == Disconnecting) && getOtherId().empty()) {
...
void CallLeg::onSipReply(const AmSipRequest &req, const AmSipReply &reply, AmSipDialog::Status old_dlg_status)
{
    ...
    TRACE("%s: SIP reply %d/%d %s (%s) received in %s state\n", getLocalTag().c_str(), reply.code, reply.cseq,
          reply.cseq_method.c_str(), (relayed_request ? "to relayed request" : "to locally generated request"),
          callStatus2str(call_status));
...
void CallLeg::addNewCallee(CallLeg *callee, ConnectLegEvent *e, AmB2BSession::RTPRelayMode mode)
{
    ...
    MONITORING_LOG4(callee->getLocalTag().c_str(), "dir", "out", "from", callee_dlg->getLocalParty().c_str(), "to",
                    callee_dlg->getRemoteParty().c_str(), "ruri", callee_dlg->getRemoteUri().c_str());
...
void CallLeg::updateCallStatus(CallStatus new_status, const StatusChangeCause &cause)
{
    if (new_status == Connected)
        TRACE("%s leg %s changing status from %s to %s with %s\n", a_leg ? "A" : "B", getLocalTag().c_str(),
              callStatus2str(call_status), callStatus2str(new_status), getOtherId().c_str());
    else
        TRACE("%s leg %s changing status from %s to %s\n", a_leg ? "A" : "B", getLocalTag().c_str(),
              callStatus2str(call_status), callStatus2str(new_status));

    setCallStatus(new_status);
    onCallStatusChange(cause);
}

void CallLeg::queueReinvite(const string &hdrs, const AmMimeBody &body, bool establishing, bool relayed_invite,
                            unsigned int r_cseq)
{
    PendingReinvite p;
    p.hdrs           = hdrs;
...
void CallLeg::acceptPendingInvite(AmSipRequest *invite)
{
    ...
    for (vector<SdpMedia>::iterator i = s.media.begin(); i != s.media.end(); ++i) {
        // i->port = 0;
        if (!i->conn.address.empty())
            i->conn.address = "0.0.0.0";
    }
...
void CallLeg::updateLocalSdp(AmSdp &sdp, const string &sip_msg_method, unsigned int sip_msg_cseq)
{
    ...
    if (dlg->getOAState() == AmOfferAnswer::OA_None || dlg->getOAState() == AmOfferAnswer::OA_Completed) {
    ...
    } else if (sip_msg_method == SIP_METH_ACK && dlg->getOAState() == AmOfferAnswer::OA_OfferRecved) {
