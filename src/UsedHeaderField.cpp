...
void UsedHeaderField::applyFormat(const string &format)
{
    ...
    if (format.starts_with("uri_user")) {
        ...
        if (param.empty()) {
            WARN("empty mandatory param for fromat '%s' for header field '%s'. use Raw", format.c_str(), name.c_str());
            type = Raw;
        } else {
            type = Uri;
            part = uri_param;
        ...
        WARN("unknown format '%s' for header field '%s'. use Raw", format.c_str(), name.c_str());
        type = Raw;
    }
...
bool UsedHeaderField::process_uri(const sip_uri &uri, string &ret) const
{
    ...
    switch (part) {
    case uri_user:   ret += c2stlstr(uri.user); return true;
    case uri_domain: ret += c2stlstr(uri.host); return true;
    case uri_port:   ret += int2str(uri.port); return true;
    case uri_param:
    {
        for (list<sip_avp *>::const_iterator i = uri.params.begin(); i != uri.params.end(); ++i) {
            const cstring &s = (*i)->name;
            if (param.length() == s.len && strncmp(s.s, param.c_str(), s.len) == 0) {
                ret += c2stlstr((*i)->value);
                return true;
            }
        }
        DBG("uri option '%s' not found in header '%s'", param.c_str(), name.c_str());
        return false;
    }
    default: ERROR("unknown part type for header '%s'", name.c_str()); return false;
    } // switch(part)
}
...
void UsedHeaderField::serialize_nameaddr(const sip_nameaddr &na, AmArg &ret) const
{
    ...
    case sip_uri::TEL:
        ...
        ret["u"] = na.uri.user.len ? c2stlstr(na.uri.user) : AmArg();
        break;
    case sip_uri::UNKNOWN: ret["s"] = "unknown"; return;
    }
...
std::optional<AmArg> UsedHeaderField::getValue(const AmSipRequest &req) const
{
    ...
    switch (type) {
    case Raw: string_ret = hdr; break;
    case Uri:
        if (parse_nameaddr_list(na_list, hdr.c_str(), hdr.length()) < 0) {
            ERROR("wrong nameaddr list '%s' in header '%s'", hdr.c_str(), name.c_str());
            return std::nullopt;
        }
        DBG("got %zd nameaddr entries. multiple_headers:%d", na_list.size(), multiple_headers);
        ...
        for (const auto &na_str : na_list) {
            ...
            if (parse_nameaddr_uri(&na, &s, na_str.len) < 0) {
                ERROR("invalid nameaddr '%.*s' in header '%s'. skip value", na_str.len, na_str.s, name.c_str());
        ...
        break;
    default: ERROR("unknown value type for header '%s'", name.c_str()); return std::nullopt;
    } // switch(type)

    if (!isArgUndef(amarg_ret)) {
        DBG("%s[%s:%s:%s] processed. got serialized value", name.c_str(), type2str(), part2str(), param.c_str());
        return amarg_ret;
    }

    if (string_ret.empty()) {
        DBG("%s[%s:%s:%s] processed. got empty value. return null", name.c_str(), type2str(), part2str(),
            param.c_str());
        return std::nullopt;
    }

    DBG("%s[%s:%s:%s] processed. got '%s'", name.c_str(), type2str(), part2str(), param.c_str(), string_ret.c_str());
