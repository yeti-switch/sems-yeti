...
{
    return AmEventDispatcher::instance()->post(
        POSTGRESQL_QUEUE, new PGExecute(PGQueryData(yeti_routing_pg_worker, query, true, /* single */
                                                    YETI_QUEUE_NAME, token),
                                        PGTransactionData()));
}
...
int YetiBase::sync_db::exec_query(const string &query, const string &token)
{
    ...
    case DB_REPLY_WAITING: throw std::logic_error("unexpected switch value");
    case DB_REPLY_RESULT:  break;
    case DB_REPLY_ERROR:
    case DB_REPLY_TIMEOUT: return 1;
    }
